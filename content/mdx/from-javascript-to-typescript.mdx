...
%%zh JavaScript 由 Brendan Eich 在 1995 年用 10 天时间在 Netscape 设计，旨在让网站易于接近和使用。从那时起，开发者们一直在拿它的怪癖和感知上的缺陷开玩笑。我将在下一节中介绍其中一些。

JavaScript has evolved tremendously since 1995, though! Its steering committee,
{{TC39|ECMAScript 的技术委员会，负责推进 JavaScript 语言提案和标准化流程。}}, has released new versions of {{ECMAScript|JavaScript 所遵循的标准化语言规范。}}—the language specification that
...
%%marginalia 这是一个较长的注释示例：用于观察当注释内容明显超过正文段落高度时，右侧是否会按“对应正文段落等高”进行裁剪。你可以继续补充更多文本来制造溢出，例如重复说明：注释应当被限制在这一段的可视高度内，不应该压到下一段对应的注释区域。
